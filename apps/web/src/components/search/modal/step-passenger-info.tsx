...
export function StepPassengerInfo({
	...
	const updatePassenger = (seatId: string, field: keyof PassengerInfo, value: string) => {
		...
		<div className="space-y-3">
			...
			<div className="space-y-3">
				{passengers.map((passenger, idx) => (
					<div
						className="rounded-xl border-2 border-border p-3 space-y-3"
					...
					>
						...
						<div className="grid gap-3 sm:grid-cols-3">
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground" htmlFor={`name-${passenger.seatId}`}>
									Full Name
								</label>
								<input
									...
									onChange={(e) => updatePassenger(passenger.seatId, "name", e.target.value)}
							...
							<div className="space-y-1">
								<label className="text-xs font-medium text-muted-foreground" htmlFor={`age-${passenger.seatId}`}>
									Age
								</label>
								<input
									className="h-9 w-full rounded-lg border bg-background px-3 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary/50"
									id={`age-${passenger.seatId}`}
									onChange={(e) => updatePassenger(passenger.seatId, "age", e.target.value)}
									placeholder="Age"
									type="number"
									value={passenger.age}
								/>
								...
								<div className="flex gap-1.5">
									{(["male", "female", "other"] as const).map((g) => (
										<button
											...
											key={g}
											onClick={() => updatePassenger(passenger.seatId, "gender", g)}
											type="button"
										>
											{g}
										</button>
